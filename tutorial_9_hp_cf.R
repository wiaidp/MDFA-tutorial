...
#   -The HP-filter assumes a particular ARIMA(0,2,2)-model for the data-generating process
...
# For lambda=1600 the MA coefficients are -1.77709 and 0.79944 (see 1.1 above)
...
#     -We avoid this problem by assigning an arbitrary (zero-) value at frequency-zero
...
#   -We want a nowcast: Lag=0. Forecast are obtained by selecting Lag<0; backcasts assume Lag>0
...
L<-len
# Use default settings for MDFA (MSE, no regularization)
# First and second order constraints are imposed
#   (the level constraint weight_constraint=1 is set in the default settings)
i1<-T
i2<-T
# Cutoff: the frequency at which the target drops below 0.5 (irrelevant in a MSE scenario)
cutoff<-pi*which(Gamma<0.5)[1]/length(Gamma)
# Nowcast
Lag<-0
# Constraints: amplitude is one, shift is zero. 
#   Other shift_constraint can be considered, leading to a right-shift (lag) or left-shift (lead) of the nowcast
#   weight_constraint is a scaling: 1 means that the trend is passed in original scale
weight_constraint<-1
shift_constraint<-0
